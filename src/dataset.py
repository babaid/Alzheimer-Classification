...
class AudioDataset(Dataset):
    def __init__(self, root_dir, transform=None):
        ...
        self.audios, self.labels = self._make_dataset(self.root_dir, self.class_id)
    ...
    def __getitem__(self, idx):
        ...
        audio, self.sample_rate  = torchaudio.load(self.audios[idx])
        length = round(len(audio[0])/2)
        
        if self.transform:
            return {"audio":self.transform(torch.real(torch.fft.fft(audio[0][0:length]))), "label":self.labels[idx]}
        else:
            return {"audio": torch.real(torch.fft.fft(audio[0][0:length])), "label":self.labels[idx]}
